...
class DBConnect {
	...
	function query($sql, $unbuffered = false) {
		...
		if ($this->query_result) {
			...
			$this->error_message = @sqlite_error_string($this->error_no);
	...
	function escape($string) {
		return is_array($string) ? '' : sqlite_escape_string($string);
	}
	...
	function exists_table($table_name) {
		$result = $this->query('SELECT 1 FROM sqlite_master WHERE name = \''.$this->prefix.$this->escape($table_name).'\' AND type=\'table\'');
	...
	function exists_index($table_name, $index_name) {
		$result = $this->query('SELECT 1 FROM sqlite_master WHERE tbl_name = \''.$this->prefix.$this->escape($table_name).'\' AND name = \''.$this->prefix.$this->escape($table_name).'_'.$this->escape($index_name).'\' AND type=\'index\'');
		return $this->num_rows($result) > 0;
	}
	...
	function rename_table($old_table, $added_table) {
		...
		if (!$this->exists_table($old_table))
			return false;
		// If the table names are the same
		elseif ($old_table == $added_table)
			return true;
		// If the new table already exists
		elseif ($this->exists_table($added_table))
			return false;

		$table = $this->get_table_info($old_table);

		// Add new table
		$query = str_replace('CREATE TABLE '.$this->prefix.$this->escape($old_table).' (', 'CREATE TABLE '.$this->prefix.$this->escape($added_table).' (', $table['sql']);
		$result = $this->query($query) ? true : false;

		// Readd indexes
		if (!empty($table['indices'])) {
			foreach ($table['indices'] as $cur_index) {
				$query = str_replace('CREATE INDEX '.$this->prefix.$this->escape($old_table), 'CREATE INDEX '.$this->prefix.$this->escape($added_table), $cur_index);
				$query = str_replace('ON '.$this->prefix.$this->escape($old_table), 'ON '.$this->prefix.$this->escape($added_table), $query);
				$result &= $this->query($query) ? true : false;
			}
		}

		// Copy content across
		$result &= $this->query('INSERT INTO '.$this->prefix.$this->escape($new_name).' SELECT * FROM '.$this->prefix.$this->escape($old_name)) ? true : false;
	...
	function add_field($table_name, $field_name, $field_type, $allow_null, $default_value = null, $after_field = null) {
		...
		$added_table = 'CREATE TABLE '.$this->prefix.$this->escape($table_name).' (';
		...
		$added_table = trim($added_table, ',')."\n".');';

		// Delete old table
		$result &= $this->delete_table($table_name);

		// Add new table
		$result &= $this->query($added_table) ? true : false;
	...
	function delete_field($table_name, $field_name) {
		...
		$added_columns = array_keys($table['columns']);
		...
		// Copy content back
		$result &= $this->query('INSERT INTO '.$this->prefix.$this->escape($table_name).' SELECT '.implode(', ', $added_columns).' FROM '.$this->prefix.$this->escape($table_name).'_t'.$now) ? true : false;
